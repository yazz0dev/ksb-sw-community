...
window.addEventListener('online', () => {
    ...
    if (appInstance) { // Check if app is mounted
        ...
        try {
            ...
            if (user) {
                ...
                if (userStore.uid !== user.uid || isInitialAuthCheck) {
                    console.log(`Fetching user data (UID: ${user.uid}, Initial: ${isInitialAuthCheck})`);
                ...
                // Clear only if currently authenticated or it's the initial check
                 if (userStore.isAuthenticated || isInitialAuthCheck) {
                    console.log(`Clearing user data (Authenticated: ${userStore.isAuthenticated}, Initial: ${isInitialAuthCheck})`);
            ...
            if (!user) { await userStore.clearUserData(); }
             if (isInitialAuthCheck) { userStore.setHasFetched(true); } // Mark fetched even on error
            notificationStore.showNotification({ message: "Error processing login/logout state.", type: 'error' });
        } finally {
             ...
             if (appInstance && !user && router.currentRoute.value.meta.requiresAuth) {
                ...
                await router.replace({ name: 'Landing' });
                 ...
                 console.warn("Auth state mismatch: Firebase has user, store doesn't. Re-fetching.");
