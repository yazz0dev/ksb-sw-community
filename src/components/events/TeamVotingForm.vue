...
const isFormValid = computed(() => {
  if (!eventCriteria.value || eventCriteria.value.length === 0) { // Check if there are any votable criteria
    // If there are no criteria to vote on, but best performer is required,
    // the form might still be valid if only best performer selection is needed.
    // However, typically team rating implies criteria.
    // If eventCriteria is empty, this part of the form is trivially filled.
    // The main check will be if bestPerformerFilled is true.
    // If there must be at least one criterion, this logic needs adjustment.
